...
public enum TestHelper {
     ...
     * @param subscriber the target
     */
    public static void doubleOnSubscribe(io.reactivex.Observer<?> subscriber) {
        List<Throwable> errors = trackPluginErrors();
        try {
     ...
     */
    enum NoOpConsumer implements Subscriber<Object>, io.reactivex.Observer<Object>, MaybeObserver<Object>, SingleObserver<Object>, CompletableObserver {
        INSTANCE;
     ...
     * @param transform the transform to drive an operator
     */
    public static <T, R> void checkDoubleOnSubscribeObservable(Function<io.reactivex.Observable<T>, ? extends ObservableSource<R>> transform) {
        List<Throwable> errors = trackPluginErrors();
        try {
            final Boolean[] b = { null, null };
            final CountDownLatch cdl = new CountDownLatch(1);

            io.reactivex.Observable<T> source = new io.reactivex.Observable<T>() {
                @Override
     ...
     * @param transform the transform to drive an operator
     */
    public static <T, R> void checkDoubleOnSubscribeObservableToSingle(Function<io.reactivex.Observable<T>, ? extends SingleSource<R>> transform) {
        List<Throwable> errors = trackPluginErrors();
        try {
            final Boolean[] b = { null, null };
            final CountDownLatch cdl = new CountDownLatch(1);
     ...
     * @param transform the transform to drive an operator
     */
    public static <T, R> void checkDoubleOnSubscribeObservableToMaybe(Function<io.reactivex.Observable<T>, ? extends MaybeSource<R>> transform) {
        List<Throwable> errors = trackPluginErrors();
        try {
            final Boolean[] b = { null, null };
            final CountDownLatch cdl = new CountDownLatch(1);
     ...
     * @param transform the transform to drive an operator
     */
    public static <T> void checkDoubleOnSubscribeObservableToCompletable(Function<io.reactivex.Observable<T>, ? extends CompletableSource> transform) {
        List<Throwable> errors = trackPluginErrors();
        try {
            final Boolean[] b = { null, null };
            final CountDownLatch cdl = new CountDownLatch(1);
     ...
     */
    public static <T> void checkFusedIsEmptyClear(io.reactivex.Observable<T> source) {
        final CountDownLatch cdl = new CountDownLatch(1);

        final Boolean[] state = { null, null, null, null };

        source.subscribe(new io.reactivex.Observer<T>() {
            @Override
            public void onSubscribe(Disposable d) {
                try {
     ...
     */
    public static <T> void checkBadSourceObservable(Function<io.reactivex.Observable<T>, Object> mapper,
            final boolean error, final T goodValue, final T badValue, final Object... expected) {
        List<Throwable> errors = trackPluginErrors();
        try {
            io.reactivex.Observable<T> bad = new io.reactivex.Observable<T>() {
                boolean once;
                @Override
                protected void subscribeActual(io.reactivex.Observer<? super T> observer) {
